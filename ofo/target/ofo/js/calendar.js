...
function getCalendar(CONTROL) {
	EndYear = new Date().getFullYear() - 17;
	StartYear = new Date().getFullYear() - 70;
	showCalendar(CONTROL, StartYear, EndYear, 'mm/dd/yyyy');
}
...
function showCalendar(CONTROL, START_YEAR, END_YEAR, FORMAT) {
	...
	var strFeatures = "width=" + CalWidth + ",height=140" + ",left=" + LEFT
			+ ",top=" + TOP;
	var CalWindow = window.open("../jsp/HTMLCalendar.html", "Calendar",
			strFeatures)
	CalWindow.focus();
} // End Function
