...
public class OrderModel {
	...
	public long nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(oid) FROM `order`");
	 ...
	 * 
	 */
	public long generateOrder(OrderDTO bean) throws ApplicationException, DuplicateRecordException
	{
		log.debug("Model generate order Started");
		Connection conn = null;
		long pk = 0;
		...
		try {
			...
			pstmt.setTimestamp(4, bean.getTime());
			
			pstmt.setLong(5, bean.getBill());
